...
describe("question Api", () => {
  ...
  describe("GET /questions/:id", () => {
    ...
    it("it should get a  question by id", async () => {
      let response = await chai
        .request(server)
        .get("/api/v1/questions/" + questionModel.id)
        .set("x-auth-token", validToken)
        .send();
      response.should.have.status(200);
      response.body.should.be.a("object");
  ...
  describe("GET /questions/userquestions", () => {
    user_id = 2;
    const userQuestions = [];

    before("set up", () => {
      const userdb = sandbox.stub(models.question, "findAll");
      userdb.withArgs({ where: { user_id } }).returns(userQuestions);
    });
    ...
    it("it should get all  question by a user", async () => {
      let response = await chai
        .request(server)
        .get("/api/v1/questions/userquestions")
        .set("x-auth-token", validToken)
        .send(userQuestions);
      response.should.have.status(200);
      response.body.should.be.an("array");
